...
// gold pill every time. Cycles automatically if there are more ranks than
// themes defined here. Kept identical to the user-facing RanksPage so both
...
// run-on sentence, so the card (and modal subtitle) can render tags.
      ...
      <div className="rank-holders-page">
        ...
        <div className="rank-holders-card">
          <div className="rank-holders-card-heading">
            <div>
              <h2>All Ranks</h2>
              <p>Explore each configured rank and the members who reached it.</p>
            </div>
          </div>
          ...
          {loadingRanks ? (
            <div className="rank-holders-empty">Loading ranks...</div>
          ) : ranks.length === 0 ? (
            <div className="rank-holders-empty">No ranks configured.</div>
          ) : (
                ...
                return (
                  ...
                  >
                    <div className="rank-card-header">
                      <span className="rank-card-icon">
                        <RankIcon aria-hidden="true" />
                      </span>
                      <div className="rank-card-title">
                        <span className="rank-card-no">Rank {index + 1}</span>
                        <span className="rank-card-name">{rank.rank_name || "-"}</span>
                      </div>
                    </div>

                    <div className="rank-criteria-tags rank-criteria-tags--card">
                      {criteriaTags.map((tag, tagIndex) => (
                        <span className="rank-criteria-tag" key={`${rank.id}-${tagIndex}`}>
                          {tag}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="rank-card-footer">
                      <span className="rank-card-holders">
                        <FiUsers aria-hidden="true" />
                        View holders
                      </span>
                      <button
                        type="button"
                        className="rank-card-arrow-btn"
                        aria-label={`View ${rank.rank_name} holders`}
                        onClick={(e) => {
                          e.stopPropagation();
                          setSelectedRankId(String(rank.id));
                        }}
                      >
        ...
        {selectedRank && selectedTheme && createPortal(
          <div className="rank-holders-modal-backdrop" onClick={() => setSelectedRankId("")}>
            <div className="rank-holders-modal" onClick={(event) => event.stopPropagation()}>
              <div className="rank-holders-modal-header">
                <div className="rank-holders-modal-heading-main">
                  <span
                    className="rank-holders-modal-icon"
                    style={{ "--rank-from": selectedTheme.from, "--rank-to": selectedTheme.to }}
                  >
                    <selectedTheme.icon aria-hidden="true" />
                  </span>
                  <div>
                    <h2>{selectedRank.rank_name} Holders</h2>
                    <div className="rank-criteria-tags rank-criteria-tags--modal">
                      {getRankCriteriaTags(selectedRank).map((tag, tagIndex) => (
                        <span className="rank-criteria-tag" key={`modal-${tagIndex}`}>
                          {tag}
                        </span>
                      ))}
                    </div>
                ...
                <div className="rank-holders-modal-heading-actions">
                  <strong className="rank-holder-count">{holders.length} holder{holders.length === 1 ? "" : "s"}</strong>
                  <button type="button" className="rank-holders-close" onClick={() => setSelectedRankId("")} aria-label="Close holders"><FiX /></button>
                </div>
              </div>
                  ...
                  <div className="holders-grid">
                    {holders.map((holder, index) => {
                      ...
                      return (
                        ...
                        >
                          <span className="holder-rank-badge" aria-hidden="true">
                            <selectedTheme.icon />
                          </span>
                          <div
                            className="holder-avatar-wrap"
